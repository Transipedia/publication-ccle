...
plotScatter <- function(k) {
    cat(k, "\n")
    tab.reindeer <- read.table(paste0(INDIR, "kmerator-reindeer/SEQC-genes_on_SEQC_raw_counts_k", k, "_kmerator.out"),
                               header = TRUE, sep = "\t") %>%
    ...
    corr.pearson <- cor(cmp.tab[, "Counts.true"], cmp.tab[, "Mean.reindeer"], method = "pearson")
    corr.spearman <- cor(cmp.tab[, "Counts.true"], cmp.tab[, "Mean.reindeer"], method = "spearman")
    # Plot scatter density
    plt <- ggplot() +
        geom_pointdensity(aes(x = cmp.tab[, "Counts.true"] + 1,
                              y = cmp.tab[, "Mean.reindeer"] + 1),
                          size = 0.1) +
        geom_text(aes(x = Inf, y = 1, 
                      label = paste0("Pearson: ", round(corr.pearson, 2), "\n",
                                     "Spearman: ", round(corr.spearman, 2))),
                  size = 5, hjust = 1, vjust = 0) +
        xlab("MAQC qPCR + 1") +
        ylab(paste0("Reindeer Query Mean (k = ", k, ") + 1")) +
        scale_x_log10() +
        scale_y_log10() +
        theme_bw() +
...
plt_lst <- lapply(c(21, 31, 41), FUN = function(k) plotScatter(k))

ggsave(plt_lst[[1]] | plt_lst[[2]] | plt_lst[[3]],
       filename = paste0(OUTDIR, "SupplFig_kmerator-reindeer.var-k.mean.png"),
