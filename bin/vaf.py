...
def compute(args):
    samples = []
    results = []
    results.append("\t".join(["gene", "mutation", "sample", "rdeer_AF", "rdeer_VAF", "depmap_AF", "depmap_VAF", "rdeer_alt_AF/depmap_alt_AF", "rdeer_VAF/depmap_VAF", "status"]))
    ...
    for mut, counts_alt in alt_dict.items():
        ...
        for i, sample in enumerate(samples):
            ...
            ### create new row with first values
            ...
            if dep_key in depmap:
                ...
                if depmap[dep_key]['vaf'] != "unknown":
                    ...
                    dep_vaf = depmap[dep_key]['vaf']
                    vaf2vaf = round(vaf / dep_vaf, 2)
            ...
            ### Add Status
            if vaf == 0 and dep_vaf == '_':
                status = "True-"
            elif vaf == 0 and dep_vaf != '_':
                status = "False-"
            elif vaf > 0 and dep_vaf != '_':
                status = "True+"
            elif vaf > 0 and dep_vaf == '_':
